class ItemsController < ApplicationController
  def index
    ...
    @items = @item_group.items.available.available_in(:shop).available_for(current_character).paginate(
      :page     => params[:page],
      :per_page => Configuration[:item_show_basic]
    )
    
    @special_items = @item_group.items.available.available_in(:special).available_for(current_character).all(

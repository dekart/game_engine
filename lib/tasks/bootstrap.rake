def img(name)
  File.open(File.join(RAILS_ROOT, "db", "pictures", "#{name}.jpg"))
end

namespace :app do
  desc "Bootstrap application data"
  task :bootstrap => :environment do
    @missions = {
      "Дератизация" => {
        :level        => 1,
        :description  => "Гигантские крысы из подземелий Столицы начали нападать на мирных людей. Убейте всех крыс и спасите мирных жителей!",
        :success_text     => "Вы убили гигантскую крысу!",
        :failure_text    => "Гигантская крыса оказалась слишком сильной, вам пришлось бежать!",
        :complete_text  => "Вы убили последнюю крысу, в коридорах больше не слышно писка.",
        :win_amount   => 5,
        :title => "Крысолов",
        :success_chance => 50,
        :ep_cost      => 1,
        :experience   => 1,
        :money_min    => 50,
        :money_max    => 100
      },
      "Воришки" => {
        :level        => 1,
        :description  => "В городе орудует банда карманников, крадущих у людей честно заработанные медяки. Поймайте подонков и предайте их суду!",
        :success_text     => "Вы поймали воришку!",
        :failure_text    => "На помощь воришке подошли подельники, вы предпочли сбежать!",
        :complete_text  => "Горожане больше не жалуются воров, спокойствие восстановлено.",
        :win_amount   => 20,
        :title => "Гроза воров",
        :ep_cost      => 2,
        :experience   => 1,
        :money_min    => 70,
        :money_max    => 150
      },

      # ---- 
      "Усмирение минотавров" => {
        :level        => 50,
        :description  => "В горном лабиринте на северной границе страны живут минотавры. Недавно они взбунтовались против справедливых налогов. Ваша задача - уничтожить зачинщиков бунта и вернуть доходы в казну",
        :success_text     => "Вы подкараулили одного из зачинщиков и задали ему взбучку!",
        :failure_text    => "Минотавр оказался неплохим бойцом, вам пришлось отступить!",
        :win_amount   => 5,
        :title => "Укротитель минотавров",
        :ep_cost      => 50,
        :experience   => 50,
        :money_min    => 1000,
        :money_max    => 5000
      }
    }

    @missions.each_pair do |key, value|
      Mission.find_or_create_by_name(key).update_attributes(value)
    end

    @weapons = {
      "Дубинка" => {
        :level => 1,
        :description => "Тяжелая дубовая палка, оружие бедняков",
        :basic_price => 50,
        :effects => Effects::Collection.new(Effects::Attack.new(1)),
        :image => img("club"),
        :placements => "left_hand,right_hand"
      },
      "Солдатский меч" => {
        :level => 1,
        :description => "Меч рядового солдата, выкованный армейскими кузнецами",
        :basic_price => 100,
        :effects => Effects::Collection.new(Effects::Attack.new(2)),
        :image => img("sword"),
        :placements => "left_hand,right_hand"
      },
      "Охотничий лук" => {
        :level => 1,
        :description => "Лук лесных охотников, простой и надежный",
        :basic_price => 70,
        :vip_price => 1,
        :effects => Effects::Collection.new(Effects::Attack.new(2)),
        :image => img("bow"),
        :availability => "loot",
        :placements => "left_hand,right_hand"
      }
    }

    @weapons.each_pair do |key, value|
      Weapon.find_or_create_by_name(key).update_attributes(value)
    end

    @armors = {
      "Кожанный жилет" => {
        :level => 1,
        :description => "Жилет из грубой бычьей кожи, закрывающий грудь и живот",
        :basic_price => 50,
        :effects => Effects::Collection.new(Effects::Defence.new(2)),
        :placements => "body"
      }
    }

    @armors.each_pair do |key, value|
      Armor.find_or_create_by_name(key).update_attributes(value)
    end
  end
end
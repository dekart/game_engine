  ...
  module Exchanges
   ...
   module ExchangeAssociationExtension 
     def accept!(exchange_offer)
       exchange_offer.exchange.transact!(exchange_offer) if exchange_offer.exchange.character == proxy_association.owner
     end
     
     def exchange_by(item)
       proxy_association.owner.exchanges.with_state(:created).first(:conditions => {:item_id => item})
     end
     
     def in_exchange?(item)
       proxy_association.owner.exchanges.with_state(:created).exists?(:item_id => item)
     end
   end

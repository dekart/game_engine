class Character
  module Missions
    def self.included(base)
      base.class_eval do
        has_one :mission_state, :extend => MissionStateExtension
    ...
    def mission_state
      (super || create_mission_state).tap do |s|
        s.character = self
      end
    end

    module MissionStateExtension
      def current_group
        GameData::MissionGroup[current_group_id] if current_group_id
      end

      def set_current_group(group)
        if group = GameData::MissionGroup[id]
    ...
    #   def current
    #     if proxy_association.owner.current_mission_group_id
    #       GameData::MissionGroup[proxy_association.owner.current_mission_group_id]
    #     end
    #   end

    #   def set_current_group(id)
    #     if group = GameData::MissionGroup[id]
    #       proxy_association.owner.update_attribute(:current_mission_group_id, group.id)
    #     end
    #   end

class Character
  module Monsters
    ...
    module MonsterFightsAssociationExtension
      ...
      def active
        scoped(:conditions => {:id => $redis.smembers(redis_key(:active)) })
      end
      
      def defeated
        $redis.zremrangebyscore(redis_key(:defeated), 0, Time.now.to_i)
        
        scoped(:conditions => {:id => $redis.zrange(redis_key(:defeated), 0, -1) })
      end
      
      def finished
        $redis.zremrangebyscore(redis_key(:finished), 0, Time.now.to_i)
        
        scoped(:conditions => {:id => $redis.zrange(redis_key(:finished), 0, -1) })
      end

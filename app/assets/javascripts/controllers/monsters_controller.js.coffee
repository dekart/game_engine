...
# obsolete controller!
window.MonstersController = class extends BaseController
  #el: '#content_wrapper'
  ...
  constructor: (monster_data, fight_data, fighters)->
    super
    ...
    @fighters = MonsterFighter.populate(fighters)
  ...
  renderImpact: ()=>
    @impact_el.html(
      @.renderTemplate('monsters/impact', @)
    )


  renderFighters: ()=>
    @fighters_el.html(
      @.renderTemplate('monsters/fighters', @)
    )

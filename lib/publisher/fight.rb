module Publisher
  class Fight < Facebooker::Rails::Publisher
    ...
    def attack_template
      one_line_story_template "{*actor*} attacked {*victim*} in #{link_to(fb_app_name, root_url(:canvas => true))} game and WON the battle!"
      short_story_template(
        "{*actor*} attacked {*victim*} in #{link_to(fb_app_name, root_url(:canvas => true))} game and WON the battle!",

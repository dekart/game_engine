...
;(function($) {
  $.facebox = function(data, options) {
    ...
    if (data.ajax) fillFaceboxFromAjax(data.ajax, options)
    else if (data.image) fillFaceboxFromImage(data.image, options)
    else if (data.div) fillFaceboxFromHref(data.div, options)
    else if ($.isFunction(data)) data.call($)
    else $.facebox.reveal(data, options)
  }
  ...
  $.extend($.facebox, {
    ...
    reveal: function(data, options) {
      ...
      if (options.klass) $('#facebox .content').addClass(options.klass);
  ...
  $.fn.facebox = function(settings) {
    ...
    function clickHandler() {
      ...
      var options = {}
      options.klass = this.rel.match(/facebox\[?\.(\w+)\]?/)
  ...
  function fillFaceboxFromHref(href, options) {
    ...
    if (href.match(/#/)) {
      ...
      $.facebox.reveal($(target).show().replaceWith("<div id='facebox_moved'></div>"), options)
  ...
  function fillFaceboxFromImage(href, options) {
    ...
    image.onload = function() {
      $.facebox.reveal('<div class="image"><img src="' + image.src + '" /></div>', options)
    }
    image.src = href
  }

  function fillFaceboxFromAjax(href, options) {
    $.get(href, function(data) { $.facebox.reveal(data, options) })
  }
